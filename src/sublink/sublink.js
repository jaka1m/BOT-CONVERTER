...
const handleMessage = async (bot, msg, userSessions, fileExtensions) => {
    ...
    if (session.step === "choose_bug") {
        ...
        const url = `https://joss.krikkrik.xyz/vpn/${session.aplikasi}?type=${session.typeconfig}&bug=${session.bug}&tls=${session.tls}&wildcard=${session.wildcard}&limit=${session.limit}&country=${session.country}`;
